...
![专注计时：倒计时、自选延长、项目进度和当天时间轴](docs/images/focus.webp)
...
- **日常提醒**：按本地时钟在整点和半点提醒喝水，使用独立提示音；喝水、起身护眼和暂停提醒可分别关闭。
- **完成记录**：结束后记下实际完成的内容，可跳过、补写，随历史与 Markdown 总结保留。
- **本地保存**：无需注册，记录和设置保存在自己的电脑上，不上传云端。

网站屏蔽在应用运行且学习日未结束时生效，关闭窗口到后台、暂停和休息仍继续屏蔽；收工或完整退出后解除。再次打开未结束的学习日会恢复屏蔽。整站屏蔽需要管理员授权，精确网址屏蔽需要安装扩展。
整站屏蔽每次开工与收工都要输一次管理员密码。macOS 上可以装一个免密助手来免掉它，代价与做法见[使用说明](docs/usage.md#免掉整站屏蔽的密码框macos)。
