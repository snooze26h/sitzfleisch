...
**时间折页界面**。骨白工作页承载计时与任务，石墨色区域承载导航和配额，朱红切面标记主要操作。
开始、暂停和归档使用专属折带素材，历史记录与核心导航沿用同一折页图形；图片随应用本地打包。
素材来源、精确提示词与本轮验证见 [视觉更新记录](design/visual-refresh-20260909/README.md)。

**菜单栏常驻**。窗口关掉也不退出。菜单栏标题跟着状态走：跑的时候是倒计时，休息是休息倒计时，
停了是「哪一格 + 停了多久」，闲着是「下一格 X」。菜单里能开始一天、开格、记水、退出。
macOS 的倒计时使用系统等宽数字；暂停时在 S 图标内显示朱红暂停角标，标题保留项目名和暂停计时，
不再额外插入「暂停」二字。跨小时显示为 `1h05`，悬浮提示保留完整时间和状态。
相同标题不重复刷新，图标只在暂停状态切换时更新。菜单栏空间不足时，系统仍可能隐藏项目。
排查过程、宽度测量与验证边界见 [菜单栏稳定性记录](design/menu-bar-stability-20260914.md)。
